...
using namespace vendor::demo::vehiclevendorextension::V1_0;
...
int main(int argc, char const* argv[])
{
    ...
    ALOGI("VHAL %s(): Hello World form demo VHAL!\n", __func__);
    ...
    ALOGI("VHAL %s(): Registering as service...", __func__);
    status_t status = service->registerAsService();

    if (status != OK)
    {
        ALOGE("VHAL %s(): Unable to register vehicle service (%d)", __func__, status);
        return 1;
    }

    ALOGI("VHAL %s(): Vehicle service ready", __func__);
    joinRpcThreadpool();
